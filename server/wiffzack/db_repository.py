...
class DatabaseRepository:
    """Provides a high-level API for application-specific database operations."""

    def __init__(self, db_connection: DatabaseConnection) -> None:
        self.db: DatabaseConnection = db_connection

    def close(self) -> None:
        """Convenience method to close the underlying database connection."""
        self.db.close()
    ...
    ********************"""
    ...
    def update_storage(self, sm: StorageModifier, absolute: bool = False) -> None:
        ...
        try:
            ...
            result: list[tuple[Any, ...]] | None = self.db.execute_query(
                query, (article.id,))
            ...
            if count == 0:
                ...
                self.execute_query(
                    query, (storage_id, storage_article_id, 0, 0, 0))  # type: ignore
            ...
            if absolute:
                ...
                self.db.execute_query(query, (storage_id, storage_article_id))
    ...
    def execute_query(self, query: LiteralString, params: Iterable[Any] = ()) -> list[tuple[Any, ...]] | None:
        """A temporary shim for existing calls. Should be removed after refactoring."""
        logger.warning(
            "Direct execute_query on Repository is deprecated. Use the method on the connection object.")
    ...
    def get_all_storage_article_groups(self) -> list[tuple[Any, ...]] | None:
        ...
        """
        rows: list[tuple[Any, ...]] | None = self.db.execute_query(query)
        return rows
    ...
    def get_storage_articles_by_group(self, article_group_id: int) -> DBResult:
        ...
        """
        rows: DBResult = self.db.execute_query(query, (article_group_id,))
        return rows
    ...
    def get_storage_articles_in_storage(self, storage_id: int) -> list[tuple[Any, ...]] | None:
        ...
        """
        rows: list[tuple[Any, ...]] | None = self.db.execute_query(
            query, (storage_id,))
        return rows
    ...
    def empty_storage(self, storage_id: int) -> None:
        ...
        """
        self.db.execute_query(query, (storage_id,))
        self.commit()
    ...
    ********************"""
    ...
    def get_client_sales(self, client: str) -> DBResult:
        ...
        """
        rows: DBResult = self.db.execute_query(query, (client,))
        return rows
    ...
    def get_latest_tallied_articles(self, client: str) -> DBResult:
        ...
        """
        rows: DBResult = self.db.execute_query(query, (f'%{client}%',))
        return rows
    ...
    def get_wardrobe_sales(self) -> DBResult:
        ...
        """
        rows: DBResult = self.db.execute_query(query)
        return rows
    ...
    ********************"""
    ...
    def get_invoice_data(self, invoice_id: int) -> DBResult:
        query: LiteralString = """
            ...
            and rechnung_id = %s
        """
        rows: DBResult = self.db.execute_query(query, (invoice_id,))
        return rows
    ...
    def get_invoice_list(self, waiter: str | None = None, invoice_type: int | None = None, limit: int = 10) -> DBResult:
        ...
        rows: DBResult = self.db.execute_query(
            cast(LiteralString, query), params)
        return rows
